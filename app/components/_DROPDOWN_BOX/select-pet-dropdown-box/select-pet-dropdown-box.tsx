...
import { PreMed14, PreReg16 } from "../../_BASIC/custom-texts/custom-texts"
          ...
          {inBottomSheet ? (
            <BottomSheetFlatList
              ...
              renderItem={({ item, index }) => {
                const isSelected = selectedPets
                  .map((selectedPet) => selectedPet.id)
                  .includes(item.id)
                return (
                  <SelectPetItem
                    petData={item}
            ...
            pets.map((item, index) => {
              const isSelected = selectedPets.map((selectedPet) => selectedPet.id).includes(item.id)
              return (
                <SelectPetItem
                  petData={item}
                  isSelected={isSelected}
